...
shapiro.test(EPAair$Ozone) # less than 0.05, not normally distributed
ggplot(EPAair, aes(x = Ozone)) +
  geom_histogram(stat = "count")
qqnorm(EPAair$Ozone); qqline(EPAair$Ozone)

O3.onesample <- t.test(EPAair$Ozone, mu = 50, alternative = "less") # is it less than 50?
O3.onesample 
```

What information does the output give us? How might we report this information in a report?

> ANWSER: low P value, reject null hypothesis. t value further away from 0, lower probability. 
95% confidence saying that the true mean is between -infinity and 37.43006.
...
var.test(EPAair$Ozone ~ EPAair$Site.Name) # the variance between the 2 population is different
...
O3.twosample
O3.twosample$p.value # reject null hypothesis, significant difference between the two means
...
bartlett.test(EPAair$PM2.5 ~ EPAair$Site.Name) # test for more multiple variances, they're not significantly different
...
> ANSWER: blakcstone has higher PM2.5 on average than bryson and triple oak.(ANOVA; p < 0.0001, df = 1989, F = 16.16)
