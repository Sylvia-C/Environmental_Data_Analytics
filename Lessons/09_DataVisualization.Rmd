...
NvsP <-
  ...
  geom_point() #+
  #geom_smooth(method = lm) + #best fit line, lm stands for linear model, include comfidence interval
  #geom_abline(aes(slope = 16, intercept = 0)) # add a specific line
print(NvsP)
...
print(OZONEvsPM)
# if you put aethestic inside geom_point, you need to specify it again in the geom_smooth  
```
...
Nutrientplot <- 
  ...
  geom_bar(aes(x = lakename, y = concentration, fill = as.factor(nutrient)), # fill define the inner color of the bars
           position = "dodge", stat = "summary", fun.y = "mean")             # position prevent them from stacking, compute the means and use that as bars
print(Nutrientplot)

Nutrientplot2 <- 
  ggplot(PeterPaul.nutrient.summaries, aes(x = lakename, y = mean, fill = as.factor(nutrient))) + # notice we're using the summaries file here, so the means are already calculated
  geom_bar(stat = "identity", position = "dodge") + # stat shows the identity it's linked to since it's already computed
...
# nowadays we're moving away from bar plots since there are some problems like negatives in error bars

# Box and whiskers plot
Nutrientplot3 <-
  ggplot(PeterPaul.nutrients.gathered, aes(x = lakename, y = concentration)) +
  geom_boxplot(aes(color = nutrient)) # Why didn't we use "fill"? because it would only color the inside of the dots/erorr bars and the points would be all grey
print(Nutrientplot3)
...
Nutrientplot6 <-
  ...
  geom_freqpoly(aes(x = tn_ug), color = "black") + # frequency polygon, need to specify color for each
...
EPAair_summary <- EPAair %>%
  group_by(Site.Name, Y) %>%
  summarise(sd = sd(PM2.5,na.rm = TRUE),
            mean = mean(PM2.5, na.rm = TRUE))
PM2.5_year_bar <-
  ggplot(EPAair_summary, aes(x = as.factor(Y), y = mean, fill = Site.Name)) +
  geom_bar(stat = "identity", position = "dodge") +
  geom_errorbar(aes(ymin = mean-sd, ymax = mean+sd), position = "dodge")
print(PM2.5_year_bar)

# 2. 
# Create a new plot that better depicts the distribution of PM2.5 concentrations. 
# Divide your graph by year and site.
PM2.5_year_boxplot <-
  ggplot(EPAair_summary, aes(x = as.factor(Y), y = mean)) +
  geom_boxplot(aes(color = Site.Name)) # still working on
print(PM2.5_year_boxplot)
