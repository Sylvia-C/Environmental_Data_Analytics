...
NvsP2 <-
  ggplot(PeterPaul.chem.nutrients, aes(x = tp_ug, y = tn_ug, color = depth, shape = lakename)) +
  geom_point(alpha = 0.8, size = 3) + # alpha adjust transparency
  xlab("TP (\U003BCg/L)") +
  ylab("TN (\U003BCg/L)") +
  #labs(x="TP (\U003BCg/L)", y="TN (\U003BCg/L)", fill="Depth", shape="lake")
  # change your legend labels here
  scale_shape_manual(values = c(21, 25)) + # the number refers to different shape, check online
