...
summary(irradiance.regression) #irridiance = 487-96(depth)+error
...
> ANSWER: irridiance is predicted around 487 at the surface, decrease 96 with every unit with depth. (linear regression; R2 = 0.31, p < 0.0001, DF = 15449)
...
irradiancebydepth <- 
  ...
  ylim(0, 2000) + #might want to remove the outlier
  geom_point() 
...
summary(irradiance.regression2) #log(irridiance) = 6.2-0.74(depth)+error
...
summary(TPregression) #TP = 6+1.5(depth)+0.94(D.O.)+error, but it only explains 8% of the data

TPplot <- ggplot(PeterPaul.chem.nutrients, 
                 aes(x = dissolvedOxygen, y = tp_ug, color = depth)) +
  geom_point() +
  xlim(0, 20)
print(TPplot) #linear model is not a good predictor in this case
```
### Correlation Plots
...
step(TPAIC) #AIC value lower the better
...
summary(TNancova.main) #TN = 353[paul lake at the surface]+136(peter lake)-9.7(depth)+error

# interaction effects
TNancova.interaction <- lm(data = PeterPaul.chem.nutrients, tn_ug ~ lakename * depth)
summary(TNancova.interaction) #TN = 325[paul lake at the surface]+185(peter lake)+20(paul)(depth)-48(peter)(depth)+error
