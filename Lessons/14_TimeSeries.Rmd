...
mk.test(Peter.nutrients.surface$tp_ug) #not really looking at dates, just looking at relative position
#p<0.0001, reject null so there is a trend, the further the z score from 0, the stronger the relationship
...
pettitt.test(Peter.nutrients.surface$tp_ug) #p<0.0001, reject null and there is a change point, at k=35

# Run separate Mann-Kendall for each change point
mk.test(Peter.nutrients.surface$tp_ug[1:34]) #not detected a trend
mk.test(Peter.nutrients.surface$tp_ug[35:132]) #not detected a trend, might be obscured by a increase and decrease

# Is there a second change point?
pettitt.test(Peter.nutrients.surface$tp_ug[35:132]) #second change point at 35+79=114

# Run another Mann-Kendall for the second change point
mk.test(Peter.nutrients.surface$tp_ug[35:113]) #increasing trend(positive z)
mk.test(Peter.nutrients.surface$tp_ug[114:132]) #no significatn trend
...
ggplot(PeterPaul.nutrients.surface, aes(x = sampledate, y = tp_ug, color = lakename)) + 
  geom_point() +
  scale_color_manual(values = c("#7fcdbb", "#253494")) +
  geom_vline(xintercept = as.Date("1993-05-20"),
             color = "#253494", lty = 2) +
  geom_vline(xintercept = as.Date("1998-01-01"),
             color = "#253494", lty = 2)
...
> ANSWER: mean when values are usually similar, linear is connecting point with straight line, spline is smooth line
...
USGS.flow.data.timeseries <- ts(USGS.flow.data.interpolated$Discharge, 
                                start = c(2009, 1) ,frequency = 12) #monthly time series

# Run a Seasonal Mann-Kendall test
USGS.smktest <- smk.test(USGS.flow.data.timeseries)
USGS.smktest
summary(USGS.smktest) #winter has the most significant increases, not seeing increases in July
```
