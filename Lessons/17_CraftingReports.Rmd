...
the dashes difine the width of the table
...
NTL_LTER.Nutrients <- read.csv("./Data/Raw/NTL-LTER_Lake_Nutrients_Raw.csv")
head(NTL_LTER.Nutrients)
NTL_LTER.Nutrients$sampledate <- as.Date(NTL_LTER.Nutrients$sampledate, "%m/%d/%y")
```
...
```{r eval=TRUE, echo=FALSE}
Nutrients_surface <- NTL_LTER.Nutrients %>%
...
```{r eval=TRUE, echo=FALSE}
kable(TN_summary, caption = "Total N summary")
kable(TP_summary, caption = "Total P summary")
```
...
```{r eval=TRUE, echo=FALSE, fig.align="left", fig.cap="total N over time"}
ggplot(Nutrients_surface, aes(x = sampledate, y = tn_ug, color = lakename)) +
  geom_smooth() +
  scale_color_brewer(palette = "Set1") +
  labs (x = "Sample Date", y = "Total Nitrogen (μg/L)")
```
...
ggplot(Nutrients_surface, aes(x = sampledate, y = tp_ug, color = lakename)) +
  geom_smooth() +
  scale_color_brewer(palette = "Set1") +
  labs (x = "Sample Date", y = "Total Phosphorous (μg/L)")
```
