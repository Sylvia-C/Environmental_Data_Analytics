...
# matrix filtering
NTL.phys.data.surface1 <- NTL.phys.data[NTL.phys.data$depth == 0,] # the comma tells which column to choose, otherwise it would choose all the columns
...
# No, dplyr has new row names, where matrix subsetting keep the original row name
...
NTL.phys.data.1999.TuesLake <- filter(NTL.phys.data, year4 >= 1990 & year4 <= 1999, 
                                      lakename == "Tuesday Lake")

```
Question: Why don't we filter using row numbers?

> ANSWER: not undertandable in the future, you have to actually look at the data to see which row you want
...
```{r}
NTL.phys.data.depth.ascending <- arrange(NTL.phys.data, depth) # ascending by default
