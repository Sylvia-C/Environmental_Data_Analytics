...
> Answer: In certain dataset like social science, NAs are meaningful so you want to keep them, R might treat blank cell as NAs too. Or in water quality data, NA may be unditectable rather than 0, so make sure you read the readme file first.

```{r}
USGS.flow.data.complete <- na.omit(USGS.flow.data) # Delete the rows with any NA in it
dim(USGS.flow.data)
...
ggplot(USGS.flow.data.complete, aes(x = discharge.mean.approval)) +
  geom_bar() #A in the chart means approved and P means pending, depends on when you pull data from USGS
```
...
```{r, fig.height = 3, fig.width = 4}
# Histogram of mean discharge, gives a warning to use a better binwidth value
ggplot(USGS.flow.data.complete) +
  geom_histogram(aes(x = discharge.mean))

# Histogram of mean discharge,create a bin every 10 units
ggplot(USGS.flow.data.complete) +
  geom_histogram(aes(x = discharge.mean), binwidth = 10)

# Histogram of mean discharge, gives 20 bins
ggplot(USGS.flow.data.complete) +
  geom_histogram(aes(x = discharge.mean), bins = 20)

# Histogram of mean discharge, with binwidth of 10, zoom in on scale 0-500
...
```{r, fig.height = 3, fig.width = 4}
# Frequency line graph of gage height, zoom in at scale 1-10, useful to have all things in similar units
ggplot(USGS.flow.data.complete) +
...
# ploting the mean gage height, divided by categorical variables 
ggplot(USGS.flow.data.complete) +
...
```{r, fig.height = 3, fig.width = 4}
# boxplot of mean gage height, divided by approval
ggplot(USGS.flow.data.complete) +
  geom_boxplot(aes(x = gage.height.mean.approval, y = gage.height.mean))

# take gage height mean, divided by 1, discharge associated with gage height, the higher gage height, the more discharge
ggplot(USGS.flow.data.complete) +
  geom_boxplot(aes(x = gage.height.mean, y = discharge.mean, group = cut_width(gage.height.mean, 1)))

# violin plot, the horizontal lines within the shape are the quantiles
ggplot(USGS.flow.data.complete) +
